...
public class CustomUserPrincipalVO implements UserDetails {

    private ArrayList<MemberVO> userVO;

    public CustomUserPrincipalVO(ArrayList<MemberVO> userAuthes) {
    ...
    public String getPassword() { //유저 비밀번호
        return userVO.get(0).getMemPw();
    }

    @Override
    public String getUsername() {// 유저 이름 혹은 아이디
        return userVO.get(0).getMemId();
    }
